...
void System::Create()
{
    ...
    m2.hbond_list.push_back(hbond(1,8,3,3 ));
    Molecule m3;
    m3.MOL_ID=2;
    ...
    for(int i=0;i<36;i++)
    {
        ...
        for(int k=0;k<4;k++)
        {
            ...
            if(ring[i].hbond_list[k].M2<0)
            {
                ring[i].nbonds-=1;
                ring[i].hbond_list[k]=ring[i].hbond_list.back();
                ring[i].hbond_list.pop_back();
            }

            if(ring[i].hbond_list[k].M2>35)
            {
                ring[i].nbonds-=1;
                ring[i].hbond_list[k]=ring[i].hbond_list.back();
                ring[i].hbond_list.pop_back();
...
{
    char FileName[200];
    sprintf(FileName, "N%d_L%d_E_dis%.1f_T_%.3f_B%.3f_Dump.lammpstrj", NMOL, Ng, E_1, total_time, K_Break);
    ofstream out;
...
void System::WriteBond(int timestep)
{
    ofstream out;
    char FileName[200];
    sprintf(FileName, "N%d_L%d_E_dis%.1f_T_%.3f_B%.3f_Bonlist.txt", NMOL, Ng, E_1, total_time, K_Break);
    out.open(FileName, ios::app);
    out << "TIMESTEP" << endl;
    out << timestep << endl;
    out << setw(12) << "molecule1"
        << "\t" << setw(12) << "molecule2"
        << "\t" << setw(12) << "arm1"
        << "\t" << setw(8) << "arm2" << endl;
    list<hbond>::iterator it;
...
void System::WriteAggregate(int timestep)
{
    ofstream out;
    char FileName[200];
    sprintf(FileName, "N%d_L%d_E_dis%.1f_T_%.3f_B%.3f_Aggregate.txt", NMOL, Ng, E_1, total_time, K_Break);
    out.open(FileName, ios::app);
    out << "TIMESTEP" << endl;
    out << timestep << endl;
...
void System::WriteLattice(int timestep)
{
    ofstream out;
    char FileName[200];
    sprintf(FileName, "N%d_L%d_E_dis%.1f_T_%.3f_B%.3f_Lattice.txt", NMOL, Ng, E_1, total_time, K_Break);
    out.open(FileName, ios::app);
    out << "TIMESTEP" << endl;
    out << timestep << endl;
